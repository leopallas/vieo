...
//        executeModify();
    ...
    private static void executeModify() {
        ...
        // 业务代理类
        BusinessHandler securityHandler = new BusinessHandler(manager);
        // 获得代理类($Proxy0 extends Proxy implements Manager)的实例.
        Manager managerProxy = (Manager) Proxy.newProxyInstance(manager
                .getClass().getClassLoader(), manager.getClass()
                .getInterfaces(), securityHandler);
        managerProxy.modify();
    }

    private static void executeModify2() {
        // 元对象(被代理对象)
        Manager manager = new ManagerImpl();
        // 业务代理类
        BusinessHandler securityHandler = new BusinessHandler(manager);
        $Proxy0 proxy = new $Proxy0(securityHandler);
        proxy.modify();
        proxy.hashCode();
    }
